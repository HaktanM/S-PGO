...
if __name__ == "__main__":
    ...
    for run_id in range(number_of_trials):
        ...
        for iter in tqdm(range(number_of_steps), desc=f"Run ID:{run_id}"):
            ...
            # step_size = np.random.rand(1) * 0.2
            ...
            elapsed_times[run_id, iter] = elapsed_time

            # Get the error
            errors     = manager.compute_estimation_errors()
            error_sum = np.array(errors).sum()
            errors_matrix[run_id, iter] = error_sum

            # if iter == int(number_of_steps/2):
            #     manager.solver.setStepSize(0.2)

        if errors_matrix[run_id,-1]>0.5:
            diverged += 1
        print(f"Divergence Rate : {diverged} / {run_id+1}, Finalr Error : {errors_matrix[run_id,-1]}")
