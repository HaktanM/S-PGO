...
for run_id in range(errors.shape[0]):
    ax.plot(errors[run_id, :], linewidth=1.0, alpha=0.5, color="tab:gray")

# # Plot the mean trajectory with a bold and distinct style
# ax.plot(mean_error, linewidth=3, color="tab:green", linestyle='--', label="Mean Trajectory")
...
ax.grid(True, linestyle='-', color='black', alpha=0.3, linewidth=0.8)
...
plt.tight_layout()
plt.show()
plt.savefig("ErrorsVSiteartionsIncrementalPoses.pdf", bbox_inches='tight', pad_inches=0)